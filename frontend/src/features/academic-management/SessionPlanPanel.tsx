...
export const SessionPlanPanel = ({ filters, subjects, teacherId }: SessionPlanPanelProps) => {
  const { user } = useAuth();
  ...
  const plansQuery = useQuery({
    queryKey,
    queryFn: () =>
      unwrap<AcademicSessionPlanRecord[]>(api.get("/academic-management/session-plans", { params: filtersToParams(filters) }))
  });

  const createMutation = useMutation({
    mutationFn: (payload: AcademicSessionPlanInput) => unwrap(api.post("/academic-management/session-plans", payload)),
    onSuccess: () => {
  ...
  const submitMutation = useMutation({
    mutationFn: (id: string) => unwrap(api.post(`/academic-management/session-plans/${id}/submit`)),
    onSuccess: () => {
  ...
  const approveMutation = useMutation({
    mutationFn: ({ id, remarks }: { id: string; remarks?: string }) =>
      unwrap(api.post(`/academic-management/session-plans/${id}/approve`, { remarks })),
    onSuccess: () => {
  ...
  const rejectMutation = useMutation({
    mutationFn: ({ id, remarks }: { id: string; remarks: string }) =>
      unwrap(api.post(`/academic-management/session-plans/${id}/reject`, { remarks })),
    onSuccess: () => {
  ...
  const unlockMutation = useMutation({
    mutationFn: (id: string) => unwrap(api.post(`/academic-management/session-plans/${id}/unlock`)),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => unwrap(api.delete(`/academic-management/session-plans/${id}`)),
    onSuccess: () => {
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-600">Annual syllabus planning linked to lesson plans and log books.</p>
        </div>
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-3 md:grid-cols-3">
              ...
              <Input
                ...
                onChange={(event) => setForm((current) => ({ ...current, academicYearBs: event.target.value, session: event.target.value }))}
                placeholder="Academic Year"
              />
              <Input value={form.faculty} onChange={(event) => setForm((current) => ({ ...current, faculty: event.target.value }))} placeholder="Faculty" />
            </div>
                  ...
                  onChange={(event) =>
                    setForm((current) => ({
                      ...
                      units: current.units.map((row, rowIndex) =>
                        rowIndex === index ? { ...row, unitNo: Number(event.target.value) } : row
                      )
                    }))
                  }
                  placeholder="Unit No"
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setForm((current) => ({
                      ...current,
                      units: current.units.map((row, rowIndex) =>
                        rowIndex === index ? { ...row, chapterName: event.target.value } : row
                      )
                    }))
                  }
                  placeholder="Chapter / Unit Name"
                />
                <Input
                  type="number"
                  ...
                  onChange={(event) =>
                    setForm((current) => ({
                      ...current,
                      units: current.units.map((row, rowIndex) =>
                        rowIndex === index ? { ...row, estimatedTeachingHours: Number(event.target.value) } : row
                      )
                    }))
                  }
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setForm((current) => ({
                      ...current,
                      units: current.units.map((row, rowIndex) =>
                        rowIndex === index ? { ...row, tentativeCompletionMonth: event.target.value } : row
                      )
                    }))
                  }
                ...
                <Textarea
                  ...
                  onChange={(event) =>
                    setForm((current) => ({
                      ...current,
                      units: current.units.map((row, rowIndex) =>
                        rowIndex === index ? { ...row, learningOutcomes: event.target.value } : row
                      )
                    }))
                  }
            ...
            <div className="flex gap-2">
              <Button variant="outline" onClick={() => setForm((current) => ({ ...current, units: [...current.units, emptyUnit()] }))}>
                Add Unit
              </Button>
              <Button
                onClick={() =>
      ...
      {filteredPlans.length === 0 ? (
        <EmptyState title="No session plans found" description="Create a session plan to start annual syllabus planning." />
      ) : (
          ...
          <Card key={plan._id} id="academic-print-area">
            <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-slate-600">
                  {plan.teacher?.user?.fullName} · {plan.completedPercent}% complete · {plan.completedUnits}/{plan.completedUnits + plan.remainingUnits} units
                </p>
              </div>
            ...
            <CardContent className="space-y-4">
                  ...
                  <TableBody>
                    {plan.units.map((unit) => (
                      <tr key={unit._id}>
                        ...
                        <Td>
                          <Badge className={statusBadgeClass(unit.status)}>{unit.status}</Badge>
                        </Td>
                      </tr>
                    ))}
                  </TableBody>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {isAdmin && (plan.status === "SUBMITTED" || plan.status === "PENDING_APPROVAL") ? (
                  <>
                    <Button size="sm" onClick={() => approveMutation.mutate({ id: plan._id })}>
                      Approve
                    </Button>
                    <Button
                      size="sm"
                ...
                {(plan.status === "DRAFT" || plan.status === "REJECTED" || isAdmin) && (
                  <Button size="sm" variant="outline" onClick={() => deleteMutation.mutate(plan._id)}>
              ...
              {plan.attachmentUrl ? (
                <a href={plan.attachmentUrl} target="_blank" rel="noopener noreferrer" className="text-sm text-brand-700 hover:underline">
                  View session plan attachment
                </a>
              ) : null}
              <AcademicCommentsPanel entityType="SESSION_PLAN" entityId={plan._id} canComment={isAdmin || plan.status !== "APPROVED"} />
            </CardContent>
          </Card>
        ))
      )}
