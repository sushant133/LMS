...
import { BookOpen, CalendarDays, ClipboardList, FileBarChart, LayoutDashboard } from "lucide-react";
...
type Tab = "dashboard" | "session-plan" | "lesson-plan" | "log-book" | "reports";
...
export const AcademicManagementHub = () => {
  ...
  const [draftFilters, setDraftFilters] = useState<AcademicManagementFilters>(defaultAcademicFilters());
  const [appliedFilters, setAppliedFilters] = useState<AcademicManagementFilters>(defaultAcademicFilters());
  ...
  const dashboardQuery = useQuery({
    ...
    queryFn: () =>
      unwrap<AcademicManagementDashboard>(api.get("/academic-management/dashboard", { params: filtersToParams(appliedFilters) })),
  ...
  const sessionPlansQuery = useQuery({
    ...
    queryFn: () =>
      unwrap<AcademicSessionPlanRecord[]>(api.get("/academic-management/session-plans", { params: filtersToParams(appliedFilters) })),
  ...
  const lessonPlansQuery = useQuery({
    ...
    queryFn: () =>
      unwrap<AcademicLessonPlanRecord[]>(api.get("/academic-management/lesson-plans", { params: filtersToParams(appliedFilters) })),
  ...
  const logBookQuery = useQuery({
    ...
    queryFn: () =>
      unwrap<AcademicLogBookEntryRecord[]>(api.get("/academic-management/log-book-entries", { params: filtersToParams(appliedFilters) })),
  ...
  const subjects = useMemo(() => {
    ...
    return all.filter((subject) => teacherScopeQuery.data.scope.subjectIds.includes(subject._id));
  ...
  const handleExportPdf = () => {
    void downloadPdfFromElementById("academic-print-area", "academic-management-report.pdf");
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <AcademicManagementFilterBar
        ...
        onPrint={handlePrint}
        activeTab={activeTab}
        classes={(classesQuery.data ?? []).map((item) => ({ _id: item._id, name: item.name }))}
        sections={(sectionsQuery.data ?? []).map((item) => ({ _id: item._id, name: item.name, classId: item.classId }))}
        batches={(batchesQuery.data ?? []).map((item) => ({ _id: item._id, name: item.name }))}
        years={(yearsQuery.data ?? []).map((item) => ({ _id: item._id, name: item.name, batchId: item.batchId }))}
      ...
      <div className={cn(activeTab === "dashboard" ? "block" : "hidden")}>
        <AcademicManagementDashboardPanel data={dashboardQuery.data} loading={dashboardQuery.isLoading} />
      </div>
      <div className={cn(activeTab === "session-plan" ? "block" : "hidden")}>
        <SessionPlanPanel filters={appliedFilters} subjects={subjects} teacherId={teacherId} />
      </div>
      <div className={cn(activeTab === "lesson-plan" ? "block" : "hidden")}>
        <LessonPlanPanel filters={appliedFilters} subjects={subjects} teacherId={teacherId} />
      </div>
      <div className={cn(activeTab === "log-book" ? "block" : "hidden")}>
        <LogBookPanel filters={appliedFilters} teacherId={teacherId} isTeacher={isTeacher} />
      </div>
