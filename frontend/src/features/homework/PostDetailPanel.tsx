...
export const PostDetailPanel = ({
  ...
  const commentMutation = useMutation({
    mutationFn: (content: string) =>
      unwrap(api.post(`/homework/${postId}/comments`, { content })),
    onSuccess: async () => {
      setCommentText("");
  ...
  const pinMutation = useMutation({
    mutationFn: (isPinned: boolean) =>
      unwrap(api.put(`/homework/${postId}/pin`, { isPinned })),
    onSuccess: async () => {
      await queryClient.invalidateQueries({
        queryKey: ["homework-detail", postId],
      });
  ...
  const gradeMutation = useMutation({
    mutationFn: ({
      ...
      unwrap(
        api.put(`/homework/submissions/${submissionId}/grade`, {
          marks,
          feedback,
        }),
      ),
          ...
          <div className="space-y-6 p-4 sm:p-6">
            <div className="space-y-3">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {post.deadlineStatus ? (
                  <Badge className={DEADLINE_COLORS[post.deadlineStatus]}>
                    {DEADLINE_LABELS[post.deadlineStatus]}
                  </Badge>
                ) : null}
                {post.submissionStatus ? (
                  <Badge className={SUBMISSION_COLORS[post.submissionStatus]}>
                    {SUBMISSION_LABELS[post.submissionStatus]}
                  </Badge>
                ) : null}
              </div>
              ...
              <p className="text-sm text-slate-600">
                ...
                {post.topic ? ` · ${post.topic}` : ""} · Posted{" "}
                {formatPostDate(post.createdAt)}
              </p>
              {post.dueDateBs ? (
                <p className="text-sm font-medium text-slate-700">
                  Due: {post.dueDateBs} (BS)
                </p>
              ) : null}
            ...
            {post.submissionStatus === "GRADED" && post.marks !== undefined ? (
              <div className="rounded-2xl border border-indigo-200 bg-indigo-50/60 p-4 text-sm">
                <p className="font-semibold text-indigo-900">
                  Grade: {post.marks}
                  {post.maxMarks ? ` / ${post.maxMarks}` : ""}
                </p>
                {post.feedback ? (
                  <p className="mt-1 text-indigo-800">{post.feedback}</p>
                ) : null}
              </div>
            ) : null}
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <Badge
                              className={SUBMISSION_COLORS[submission.status]}
                            >
                              ...
                              <Button
                                ...
                                onClick={() =>
                                  gradeMutation.mutate({
                                    ...
                                    marks: Number(
                                      gradeMarks[submission._id] ?? 0,
                                    ),
                                    feedback:
                                      gradeFeedback[submission._id] ?? "",
                                  })
                                }
                              >
                                Grade
