...
import type { BatchRecord, ClassRecord, SectionRecord, StudentInput, StudentRecord, YearRecord } from "@phit-erp/shared";
...
import { mapStudentToInput, type StudentEditLocationState } from "./studentFormUtils";
...
export const CreateStudentManager = () => {
  ...
  const editState = (location.state as StudentEditLocationState | null)?.student ?? null;
  ...
  const studentMutation = useMutation({
    ...
    onSuccess: async (data) => {
      ...
      const profileStudentId = "loginEmail" in data ? data.student._id : data._id;
      setEditing(null);
      setPendingDocuments([]);
      await queryClient.invalidateQueries({ queryKey: ["students"] });
      await queryClient.invalidateQueries({ queryKey: ["student-profile", profileStudentId] });
    },
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{editing ? "Edit Student" : "New Student Registration"}</CardTitle>
      </CardHeader>
      <CardContent>
        <StudentForm
          ...
          onSubmit={async (value) => {
            ...
            if (!wasEditing && pendingDocuments.length > 0 && "student" in result) {
              ...
              setPendingDocuments([]);
              await queryClient.invalidateQueries({ queryKey: ["students"] });
              await queryClient.invalidateQueries({ queryKey: ["student-profile", result.student._id] });
            }
            navigate("/students/list", { replace: true });
          }}
        />
