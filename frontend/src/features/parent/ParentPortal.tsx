...
export const ParentPortal = () => {
  ...
  const { isModuleEnabled } = useParentPortalAccess();
  ...
  const access = data?.portalAccess;
  const can = (key: Parameters<typeof isModuleEnabled>[0]) => {
    if (access && typeof access[key] === "boolean") {
      return access[key] !== false;
    }
    return isModuleEnabled(key);
  };

  const showOverview = can("overview");
  const showAttendance = can("attendance");
  const showFees = can("fees");
  const showHomework = can("homework");
  const showField = can("field-attendance");
  ...
  if (!showOverview && !showHomework && !showNotifications && !showField) {
    return (
      <div className="space-y-6">
        <PageHeader
          title="Parent Portal"
          description="Your parent portal access is limited by the college administrator."
        />
        <EmptyState
          title="No portal sections available"
          description="The administrator has not enabled any parent portal sections for this college. Please contact the office."
        />
      </div>
    );
  }
                      ...
                      <>
                        ...
                        <div>
                          ...
                          <p className="font-semibold text-emerald-700">
                            {formatCurrencyNpr(
                              (child as { totalPaidNpr?: number }).totalPaidNpr ??
                                0,
                            )}
                          </p>
                        </div>
