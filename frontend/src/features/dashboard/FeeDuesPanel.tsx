...
export const FeeDuesPanel = ({
  ...
  const feeDuesQuery = useQuery({
    ...
    queryFn: () =>
      unwrap<DashboardFeeDueStudent[]>(api.get("/dashboard/fee-dues")),
    enabled: open,
  });

  const sendReminder = useMutation({
    mutationFn: (studentId: string) =>
      unwrap(api.post(`/dashboard/fee-dues/${studentId}/remind`)),
    onSuccess: async () => {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-end justify-center bg-slate-900/50 p-4 sm:items-center">
      <button
        type="button"
        aria-label="Close fee dues panel"
        ...
        <CardHeader className="flex shrink-0 flex-row items-start justify-between gap-4 space-y-0 border-b border-slate-100">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Outstanding balances, contact details, and quick actions for
              follow-up.
            </p>
          </div>
          <Button
                    ...
                    <div className="flex min-w-0 gap-4">
                      ...
                      <div className="min-w-0 space-y-1">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <Badge
                            className={
                              paymentStatusClass[student.paymentStatus]
                            }
                          >
                        ...
                        <p className="text-sm text-slate-500">
                          ID {student.admissionNumber} · Roll{" "}
                          {student.rollNumber}
                        </p>
                    ...
                    <div className="grid shrink-0 gap-2 text-sm sm:grid-cols-2 lg:min-w-[280px]">
                      <div>
                        ...
                        <p className="font-semibold text-slate-900">
                          {formatCurrencyNpr(student.totalFeeNpr)}
                        </p>
                      </div>
                      <div>
                        <p className="text-slate-500">Amount Paid</p>
                        <p className="font-semibold text-slate-900">
                          {formatCurrencyNpr(student.amountPaidNpr)}
                        </p>
                      </div>
                      <div>
                        <p className="text-slate-500">Outstanding</p>
                        <p className="font-semibold text-rose-700">
                          {formatCurrencyNpr(student.outstandingAmountNpr)}
                        </p>
                      </div>
                      <div>
                        <p className="text-slate-500">Pending Installments</p>
                        <p className="font-semibold text-slate-900">
                          {student.pendingInstallments}
                        </p>
                      </div>
                      {student.dueDateBs ? (
                  ...
                  <div className="mt-4 flex flex-wrap gap-2 border-t border-slate-100 pt-4">
                    ...
                    {canAccessAccounting ? (
                      <>
                        <Button asChild size="sm" variant="outline">
                          <Link
                            to={accountingFeeCollectionUrl(student.studentId)}
                          >
                        ...
                        <Button asChild size="sm" variant="outline">
                          <Link
                            to={accountingStudentAccountsUrl(student.studentId)}
                          >
                        ...
                        {student.lastReceiptId ? (
                          <Button
                            ...
                            onClick={() =>
                              window.open(
                                `${api.defaults.baseURL}/accounting/collections/${student.lastReceiptId}/receipt`,
                                "_blank",
                              )
                            }
                          >
