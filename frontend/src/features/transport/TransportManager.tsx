...
export const TransportManager = () => {
  ...
  const [routeForm, setRouteForm] = useState<TransportRouteInput>({
    ...
    driverName: "",
    driverPhone: "",
    stops: [{ name: "Main Gate", pickupTime: "07:30" }],
    monthlyFeeNpr: 0,
    isActive: true,
  });
  const [assignForm, setAssignForm] = useState<TransportAssignmentInput>({
    routeId: "",
    studentId: "",
    pickupStop: "",
    dropStop: "",
    ...
    queryFn: () =>
      unwrap<
        Array<{
          _id: string;
          routeId?: { name: string };
          studentId?: { _id: string; user: { fullName: string } };
          pickupStop: string;
          dropStop: string;
        }>
  ...
  const createRoute = useMutation({
    mutationFn: (payload: TransportRouteInput) =>
      unwrap(api.post("/transport/routes", payload)),
    onSuccess: async () => {
  ...
  const assignStudent = useMutation({
    mutationFn: (payload: TransportAssignmentInput) =>
      unwrap(api.post("/transport/assignments", payload)),
    onSuccess: async () => {
              ...
              <CardContent className="space-y-3">
                <FormField label="Route name">
                  <Input
                    ...
                    onChange={(e) =>
                      setRouteForm((c) => ({ ...c, name: e.target.value }))
                    }
                  />
                </FormField>
                ...
                <FormField label="Monthly fee (NPR)">
                  <NumberInput
                    value={routeForm.monthlyFeeNpr}
                    onChange={(e) =>
                      setRouteForm((c) => ({
                        ...c,
                        monthlyFeeNpr: e.target.valueAsNumber,
                      }))
                    }
                  />
                </FormField>
                <Button
                  onClick={() => {
                    const p = transportRouteSchema.safeParse(routeForm);
                    if (!p.success) return toast.error("Invalid route");
                    createRoute.mutate(p.data);
                  }}
                >
                  Create route
                </Button>
              ...
              <CardContent className="space-y-3">
                <FormField label="Route">
                  <Select
                    ...
                    onChange={(e) =>
                      setAssignForm((c) => ({ ...c, routeId: e.target.value }))
                    }
                  >
                    <option value="">Select route</option>
                    {(routesQuery.data ?? []).map(
                ...
                <FormField label="Student">
                  ...
                  >
                    ...
                    {(studentsQuery.data ?? []).map(
                      (s: { _id: string; user: { fullName: string } }) => (
                ...
                <FormField label="Drop stop">
                  <Input
                    value={assignForm.dropStop}
                    onChange={(e) =>
                      setAssignForm((c) => ({ ...c, dropStop: e.target.value }))
                    }
                  />
                </FormField>
                <Button
                  onClick={() => {
                    const p = transportAssignmentSchema.safeParse(assignForm);
                    if (!p.success) return toast.error("Invalid assignment");
                    assignStudent.mutate(p.data);
                  }}
                >
                  Assign
                </Button>
                    ...
                    <Td>
                      {a.studentId?._id && a.studentId.user?.fullName ? (
                        <StudentNameLink
                          ...
                          name={a.studentId.user.fullName}
                        />
                      ) : (
                        "—"
                      )}
