...
export const ComplaintsManager = () => {
  ...
  const [statusDraft, setStatusDraft] = useState<{ status: ComplaintStatus; adminResponse: string }>({
    status: "UNDER_REVIEW",
    adminResponse: ""
  });
  ...
  const openCount = useMemo(
    () => complaints.filter((item) => item.status === "SUBMITTED" || item.status === "UNDER_REVIEW").length,
    [complaints]
  );

  const createComplaint = useMutation({
    mutationFn: (payload: CreateComplaintInput) => unwrap(api.post("/complaints", payload)),
    onSuccess: () => {
      toast.success("Complaint submitted. Only you and the college administration can view it.");
      setForm(defaultForm);
    ...
    <PageContent className="space-y-6">
        ...
        <Card className="border-brand-100 shadow-sm">
          ...
          <CardContent className="space-y-4 pt-6">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <FormField label="Category">
                <Select
                  value={form.category}
                  onChange={(event) =>
                    setForm((current) => ({ ...current, category: event.target.value as CreateComplaintInput["category"] }))
                  }
                >
            ...
            <div className="flex justify-end border-t border-slate-100 pt-4">
              <Button onClick={handleSubmit} disabled={createComplaint.isPending}>
      ...
      <div className="space-y-3">
        ...
        {complaintsQuery.isLoading ? (
          <Card>
            <CardContent className="py-10 text-center text-sm text-slate-500">Loading complaints...</CardContent>
          </Card>
        ) : complaints.length === 0 ? (
          <EmptyState
            ...
            return (
              <Card key={complaint._id} className="overflow-hidden">
                <CardContent className="space-y-4 py-5">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div className="min-w-0 flex-1 space-y-2">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <Badge className={cn("text-xs", statusBadgeClass[complaint.status])}>
                          {COMPLAINT_STATUS_LABELS[complaint.status]}
                        </Badge>
                        <Badge className="bg-slate-100 text-slate-700">{COMPLAINT_CATEGORY_LABELS[complaint.category]}</Badge>
                      </div>
                      <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500">
                        <span>Submitted {formatDate(complaint.createdAt)}</span>
                        {canViewAll && complaint.submitterName ? <span>By {complaint.submitterName}</span> : null}
                        {canViewAll ? <span>{USER_ROLE_LABELS[complaint.submitterRole]}</span> : null}
                      </div>
                    </div>
                    ...
                    {canViewAll ? (
                      <div className="flex shrink-0 flex-wrap gap-2">
                        {canWriteAdmin ? (
                          <>
                            <Button size="sm" variant="secondary" onClick={() => (isExpanded ? setExpandedId(null) : startAdminReview(complaint))}>
                          ...
                          <Button size="sm" variant="secondary" onClick={() => (isExpanded ? setExpandedId(null) : setExpandedId(complaint._id))}>
                  ...
                  <p className="whitespace-pre-wrap text-sm leading-relaxed text-slate-700">{complaint.content}</p>

                  {complaint.attachments.length > 0 ? (
                    <AttachmentViewer attachments={complaint.attachments} title="Attached evidence" />
                  ) : null}

                  {complaint.adminResponse ? (
                    <div className="rounded-xl border border-emerald-100 bg-emerald-50/60 p-4">
                      <p className="text-xs font-semibold uppercase tracking-wide text-emerald-700">Administration response</p>
                      <p className="mt-1 whitespace-pre-wrap text-sm text-emerald-900">{complaint.adminResponse}</p>
                    ...
                    <div className="space-y-3 rounded-xl border border-slate-200 bg-slate-50 p-4">
                      <div className="grid gap-3 md:grid-cols-2">
                        <FormField label="Status">
                          <Select
                            ...
                            onChange={(event) =>
                              setStatusDraft((current) => ({ ...current, status: event.target.value as ComplaintStatus }))
                            }
                          >
                      ...
                      <FormField label="Response to complainant">
                        <Textarea
                          ...
                          onChange={(event) => setStatusDraft((current) => ({ ...current, adminResponse: event.target.value }))}
                        />
                      </FormField>
