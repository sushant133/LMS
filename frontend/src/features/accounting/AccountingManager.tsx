...
export const AccountingManager = () => {
  ...
  const receiptsQuery = useQuery({
    ...
    queryFn: () => unwrap<Array<Record<string, unknown>>>(api.get("/accounting/receipts")),
  ...
  const expensesQuery = useQuery({
    ...
    queryFn: () => unwrap<Array<Record<string, unknown>>>(api.get("/accounting/expenses")),
    enabled: tab === "expenses"
  });

  const purchasesQuery = useQuery({
    queryKey: ["accounting-purchases"],
    queryFn: () => unwrap<Array<Record<string, unknown>>>(api.get("/accounting/purchases")),
    enabled: tab === "purchases"
  });

  const incomeQuery = useQuery({
    queryKey: ["accounting-income"],
    queryFn: () => unwrap<Array<Record<string, unknown>>>(api.get("/accounting/income")),
    enabled: tab === "income"
  });

  const salariesQuery = useQuery({
    queryKey: ["accounting-salaries"],
    queryFn: () => unwrap<Array<Record<string, unknown>>>(api.get("/accounting/salaries")),
  ...
  const cashBookQuery = useQuery({
    ...
    queryFn: () => unwrap<Array<Record<string, unknown>>>(api.get("/accounting/cash-book")),
    enabled: tab === "cash-book"
  });

  const bankAccountsQuery = useQuery({
    queryKey: ["accounting-bank-accounts"],
    queryFn: () => unwrap<Array<Record<string, unknown>>>(api.get("/accounting/bank-accounts")),
  ...
  const createStructure = useMutation({
    mutationFn: (payload: ExtendedFeeStructureInput) => unwrap(api.post("/accounting/structures", payload)),
    onSuccess: async () => {
                ...
                <FormField label="Student">
                  ...
                  >
                    ...
                    {(studentsQuery.data ?? []).map((s) => (
                      <option key={s._id} value={s._id}>{s.user.fullName} — {s.admissionNumber}</option>
                    ))}
                  </Select>
                </FormField>
            ...
            <Table>
              ...
              <TableBody>
                {(receiptsQuery.data ?? []).map((row) => {
                  const student = row.studentId as { user?: { fullName?: string } } | string;
                  const studentName = typeof student === "object" ? student.user?.fullName ?? "—" : "—";
                  return (
                    <tr key={String(row._id)}>
                      <Td>{String(row.receiptNumber)}</Td>
                      <Td>{studentName}</Td>
                      <Td>{String(row.paidDateBs)}</Td>
                      <Td>{formatCurrencyNpr(Number(row.amountPaidNpr))}</Td>
                      <Td>{formatCurrencyNpr(Number(row.remainingDueNpr ?? 0))}</Td>
                      <Td>{String(row.paymentMethod ?? "CASH").replace(/_/g, " ")}</Td>
                      <Td>
                        <Button size="sm" variant="outline" onClick={() => downloadReceipt(String(row._id))}>PDF</Button>
                      </Td>
                    </tr>
                  );
                })}
        ...
        <div className="grid gap-6 xl:grid-cols-2">
          <Card>
            ...
            <CardContent className="overflow-x-auto">
              <Table>
                <TableHead>
                  <tr><Th>Student</Th><Th>Class</Th><Th>Due</Th><Th>Paid</Th><Th /></tr>
                </TableHead>
                <TableBody>
                  {(studentAccountsQuery.data ?? []).map((account) => (
              ...
              <Table>
                ...
                <TableBody>
                  {(salariesQuery.data ?? []).map((row) => {
                    const teacher = row.teacherId as { user?: { fullName?: string } } | undefined;
                    return (
                      <tr key={String(row._id)}>
                        <Td>{String(row.monthBs)}</Td>
                        <Td>{teacher?.user?.fullName ?? String(row.staffName ?? "—")}</Td>
                        <Td>{formatCurrencyNpr(Number(row.netSalaryNpr))}</Td>
      ...
      {tab === "expenses" ? (
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <Card>
            ...
            <CardContent className="overflow-x-auto">
              <Table>
                <TableHead><tr><Th>Date</Th><Th>Category</Th><Th>Vendor</Th><Th>Amount</Th></tr></TableHead>
                <TableBody>
                  {(expensesQuery.data ?? []).map((row) => (
                    <tr key={String(row._id)}>
                      <Td>{String(row.dateBs)}</Td>
                      <Td>{String(row.category)}</Td>
                      <Td>{String(row.vendor)}</Td>
                      <Td>{formatCurrencyNpr(Number(row.amountNpr))}</Td>
                    </tr>
                  ))}
                </TableBody>
              </Table>
      ...
      {tab === "purchases" ? (
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <Card>
            ...
            <CardContent className="overflow-x-auto">
              <Table>
                <TableHead><tr><Th>Date</Th><Th>Category</Th><Th>Invoice</Th><Th>Total</Th><Th>Status</Th></tr></TableHead>
                <TableBody>
                  {(purchasesQuery.data ?? []).map((row) => (
                    <tr key={String(row._id)}>
                      <Td>{String(row.purchaseDateBs)}</Td>
                      <Td>{String(row.category)}</Td>
                      <Td>{String(row.invoiceNumber)}</Td>
                      <Td>{formatCurrencyNpr(Number(row.totalAmountNpr))}</Td>
                      <Td><Badge>{String(row.paymentStatus)}</Badge></Td>
                    </tr>
                  ))}
                </TableBody>
              </Table>
      ...
      {tab === "income" ? (
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <Card>
            ...
            <CardContent className="overflow-x-auto">
              <Table>
                <TableHead><tr><Th>Date</Th><Th>Category</Th><Th>Source</Th><Th>Amount</Th></tr></TableHead>
                <TableBody>
                ...
                <TableBody>
                  {(cashBookQuery.data ?? []).map((row) => (
                    <tr key={String(row._id)}>
                      <Td>{String(row.dateBs)}</Td>
                      <Td>
                        <Badge className={row.entryType === "CREDIT" ? undefined : "bg-rose-100 text-rose-800"}>{String(row.entryType)}</Badge>
                      </Td>
                      <Td>{String(row.description)}</Td>
                      <Td>{formatCurrencyNpr(Number(row.amountNpr))}</Td>
                      <Td>{formatCurrencyNpr(Number(row.balanceAfterNpr))}</Td>
                    </tr>
                  ))}
                </TableBody>
              </Table>
                ...
                <TableBody>
                  {(bankAccountsQuery.data ?? []).map((row) => (
                    <tr key={String(row._id)}>
                      ...
                      <Td>{String(row.accountName)}</Td>
                      <Td>{String(row.accountNumber)}</Td>
                      <Td>{formatCurrencyNpr(Number(row.currentBalanceNpr))}</Td>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="overflow-x-auto">
              <Table>
                <TableHead>
                  <tr>
                    {reportQuery.data?.data?.[0]
                      ? Object.keys(reportQuery.data.data[0] as Record<string, unknown>).slice(0, 6).map((key) => <Th key={key}>{key}</Th>)
                      : <Th>No data</Th>}
                  </tr>
                </TableHead>
                <TableBody>
                  {(reportQuery.data?.data ?? []).slice(0, 20).map((row, index) => (
                    <tr key={index}>
                      {Object.values(row as Record<string, unknown>).slice(0, 6).map((value, i) => (
                        <Td key={i}>{typeof value === "number" ? formatCurrencyNpr(value) : String(value ?? "")}</Td>
                      ))}
                    </tr>
                  ))}
                </TableBody>
        ...
        <Card>
          ...
          <CardContent className="grid gap-3 md:grid-cols-2">
            {settingsQuery.data ? (
              <>
                <FormField label="Late Fine %">
                  <Input type="number" value={settingsForm.lateFinePercent || settingsQuery.data.lateFinePercent} onChange={(e) => setSettingsForm((c) => ({ ...c, lateFinePercent: Number(e.target.value) }))} />
                </FormField>
                <FormField label="Grace Days">
                  <Input type="number" value={settingsForm.lateFineGraceDays || settingsQuery.data.lateFineGraceDays} onChange={(e) => setSettingsForm((c) => ({ ...c, lateFineGraceDays: Number(e.target.value) }))} />
                </FormField>
                <FormField label="Receipt Prefix">
                  <Input value={settingsForm.receiptPrefix || settingsQuery.data.receiptPrefix} onChange={(e) => setSettingsForm((c) => ({ ...c, receiptPrefix: e.target.value }))} />
                </FormField>
                <div className="md:col-span-2">
                  <Button onClick={() => {
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <FormField label="Email"><Input type="email" value={accountantForm.email} onChange={(e) => setAccountantForm((c) => ({ ...c, email: e.target.value }))} /></FormField>
