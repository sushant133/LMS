...
import { RESULT_SUBMISSION_STATUS_COLORS, RESULT_SUBMISSION_STATUS_LABELS } from "features/exams/examDefaults";
...
const StatusBadge = ({ status }: { status: string }) => (
  <Badge className={RESULT_SUBMISSION_STATUS_COLORS[status] ?? "bg-slate-100 text-slate-700"}>
    {RESULT_SUBMISSION_STATUS_LABELS[status] ?? status}
  </Badge>
);

export const ResultReviewPanel = ({ examId, students, subjects, isCollege, compact = false }: ResultReviewPanelProps) => {
    ...
    queryFn: () =>
      unwrap<EnrichedSubmission[]>(
        api.get("/exams/result-submissions", {
          params: { examId: examId || undefined, status: statusFilter || undefined }
        })
      ),
  ...
  const selectedSubmission = useMemo(
    () => (submissionsQuery.data ?? []).find((item) => item._id === selectedSubmissionId),
  ...
  const resultsQuery = useQuery({
    ...
    queryFn: () => unwrap<ResultRecord[]>(api.get("/exams/results/all", { params: { examId: reviewExamId } })),
  ...
  const subjectResults = useMemo(() => {
    ...
    return results
      ...
      .map((result) => {
        const mark = result.marks.find((item) => String(item.subjectId) === String(selectedSubmission.subjectId));
  ...
  const reviewSummary = useMemo(() => {
    ...
    const marks = subjectResults.map(({ mark }) => computeSubjectMark({ ...mark!, obtainedMarks: 0 }));
    const passCount = marks.filter((mark) => mark.passFail === "PASS").length;
    const failCount = marks.length - passCount;
    const averagePercentage =
      marks.length > 0 ? Number((marks.reduce((sum, mark) => sum + (mark.percentage ?? 0), 0) / marks.length).toFixed(2)) : 0;
    const averageGpa =
      subjectResults.length > 0
        ? Number((subjectResults.reduce((sum, item) => sum + item.result.gpa, 0) / subjectResults.length).toFixed(2))
        : 0;

    return { passCount, failCount, averagePercentage, averageGpa, totalStudents: marks.length };
  }, [subjectResults]);

  const approveMutation = useMutation({
    mutationFn: async ({ submissionId, comments }: { submissionId: string; comments?: string }) =>
      unwrap(api.post(`/exams/result-submissions/${submissionId}/approve`, { comments })),
    onSuccess: async () => {
      toast.success("Results approved. Use Publish Results on the exam session to release them to students.");
      setReviewComments("");
  ...
  const returnMutation = useMutation({
    mutationFn: async ({ submissionId, comments }: { submissionId: string; comments: string }) =>
      unwrap(api.post(`/exams/result-submissions/${submissionId}/return`, { comments })),
    onSuccess: async () => {
  ...
  const adminEditMutation = useMutation({
    mutationFn: async (payload: unknown) => unwrap(api.post("/exams/results/admin", payload)),
    onSuccess: async () => {
    ...
    <div className="space-y-6">
      {!examId && !compact ? (
        <p className="text-sm text-slate-600">
          Showing teacher submissions across all exams. Select a submission below to review marks, approve, or return for correction.
        </p>
      ) : null}
      <div className="flex flex-wrap items-end gap-4">
        <FormField label="Filter by status">
          <Select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}>
            ...
            {Object.entries(RESULT_SUBMISSION_STATUS_LABELS).map(([value, label]) => (
      ...
      {(submissionsQuery.data ?? []).length === 0 ? (
        <EmptyState title="No submissions found" description="Teachers will appear here after submitting results for review." />
      ) : (
          ...
          <Table>
            ...
            <TableBody>
              {(submissionsQuery.data ?? []).map((submission) => (
                <tr key={submission._id} className={selectedSubmissionId === submission._id ? "bg-slate-50" : ""}>
                  <Td>
                    <div>
                  ...
                  <Td>
                    <Button size="sm" variant="outline" onClick={() => setSelectedSubmissionId(submission._id)}>
                      Review
                    </Button>
                  </Td>
                </tr>
      ...
      {selectedSubmission ? (
        <div className="space-y-4 rounded-2xl border border-slate-200 p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <h3 className="text-lg font-semibold text-slate-900">{selectedSubmission.scopeLabel}</h3>
              <p className="text-sm text-slate-600">
                {selectedSubmission.marksEntered} of {selectedSubmission.studentsTotal} students have marks entered
              </p>
          ...
          {selectedSubmission.missingStudents.length > 0 ? (
            <div className="rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-800">
              ...
              <p>{selectedSubmission.missingStudents.map((item) => item.studentName).join(", ")}</p>
            </div>
          ) : null}
          ...
          {reviewSummary ? (
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              <div className="rounded-xl border border-slate-200 bg-white px-4 py-3">
                ...
                <p className="text-lg font-semibold text-slate-900">{reviewSummary.averagePercentage}%</p>
              </div>
              <div className="rounded-xl border border-slate-200 bg-white px-4 py-3">
              ...
              <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3">
                ...
                <p className="text-lg font-semibold text-red-900">{reviewSummary.failCount}</p>
              </div>
            </div>
          ) : null}
            ...
            <Table>
              ...
              <TableBody>
                {subjectResults.map(({ result, mark, student }) => {
                  ...
                  const isEditing =
                    editingMark?.resultId === result._id && editingMark.subjectId === mark.subjectId;
                        ...
                        <>
                            ...
                            <Input
                              ...
                              onChange={(event) =>
                                setEditingMark((current) =>
                                  current ? { ...current, theoryMarks: event.target.valueAsNumber } : current
                                )
                              }
                            />
                          </Td>
                          <Td>
                            <Input
                              ...
                              onChange={(event) =>
                                setEditingMark((current) =>
                                  current ? { ...current, practicalMarks: event.target.valueAsNumber } : current
                                )
                              }
                            />
                          </Td>
                          <Td>
                            <Input
                              ...
                              onChange={(event) =>
                                setEditingMark((current) =>
                                  current ? { ...current, internalMarks: event.target.valueAsNumber } : current
                                )
                              }
                            />
                          </Td>
                          <Td colSpan={2}>
                            <div className="flex gap-2">
                              <Button
                                ...
                                onClick={() => {
                                  const subject = subjects.find((item) => item._id === mark.subjectId);
                                    ...
                                    marks: [
                                      {
                                        ...
                                        practicalMarks: editingMark.practicalMarks,
                                        internalMarks: editingMark.internalMarks,
                                  ...
                                  void adminEditMutation.mutateAsync(parsed.data);
                                }}
                              >
                                Save
                              </Button>
                              <Button size="sm" variant="outline" onClick={() => setEditingMark(null)}>
                                Cancel
                              </Button>
                            </div>
                          </Td>
          ...
          {(selectedSubmission.status === "PENDING_ADMIN_REVIEW" || selectedSubmission.status === "SUBMITTED_FOR_REVIEW") && (
            <div className="space-y-3 border-t border-slate-200 pt-4">
              <FormField label="Admin comments (required for return/reject)">
                <Textarea value={reviewComments} onChange={(event) => setReviewComments(event.target.value)} placeholder="Add review comments..." />
              </FormField>
              <div className="flex flex-wrap gap-2">
                <Button
                  disabled={approveMutation.isPending || selectedSubmission.missingStudents.length > 0}
                  onClick={() => void approveMutation.mutateAsync({ submissionId: selectedSubmission._id, comments: reviewComments })}
                >
                  Approve Results
                </Button>
                <Button
                  ...
                  onClick={() =>
                    void returnMutation.mutateAsync({ submissionId: selectedSubmission._id, comments: reviewComments })
                  }
                >
      ...
      {!compact ? (
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          {auditLogQuery.isLoading ? (
            ...
            <p className="mt-2 text-sm text-slate-500">No audit entries yet{examId ? " for this exam" : ""}.</p>
          ) : (
                ...
                <div key={entry._id} className="rounded-lg bg-slate-50 px-3 py-2 text-sm">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    <span className="text-xs text-slate-500">
                      {entry.createdAt ? new Date(entry.createdAt).toLocaleString() : ""} · {entry.actorRole}
                    </span>
                  </div>
                </div>
              ))}
